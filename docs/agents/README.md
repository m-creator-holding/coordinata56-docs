...
- Читать `agents-map.yaml`. Поля стабильны: `name`, `id`, `level`, `department`, `unit`, `role`, `purpose`, `inputs`, `outputs`, `delegates_to`, `reviewed_by`, `memory_paths`, `status_meaning`, `risks`, `notes`.
...
3. Для M/L — ответственный Директор направления.
4. Для S — ответственный Начальник отдела.
5. Для XS — либо Координатор сам, либо один Worker.
6. Если задача затрагивает несколько департаментов — см. схему Г «делегирование» в `agents-diagrams.md` + правила кросс-вертикальной коммуникации в `CODE_OF_LAWS.md` статья 13.
