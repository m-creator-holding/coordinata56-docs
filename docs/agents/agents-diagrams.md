# Визуальные схемы системы субагентов coordinata56
...
Эта схема лжёт в одном: она показывает будто `ri-director → ri-scout` — прямая стрелка, как если бы director сам запускал scout'а. Технически это не так (см. следующую схему З). Но для понимания команды и ответственности — эта схема правильная.
